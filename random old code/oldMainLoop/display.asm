...
;             outputted to the display
; 4) InitVariables: initializes segment buffer and index that points to 
...
; Stack Depth: None.
;
; Limitations: the display can only display 8 ASCII characters at at ime
;
...
NextChar: 
    CMP DI, DISPLAY_LEN ; keeps function looping if the number of items
                        ; in the buffer is < Display_Len
    JE EndDisplay
...
Convert:
    ...
    MOV segbuffer[DI], AL ; stores segment pattern in buffer
    
    INC DI ; increment current segment buffer index
...
MuxPatterns:
    MOV     BX, Digit
    MOV     AL, segbuffer[BX]  ; accesses current segment pattern to put on display
