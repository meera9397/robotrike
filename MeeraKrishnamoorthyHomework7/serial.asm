...
SetParity: 
	MOV BX, PARITY_OFF  ; sets register that is used to determine
                        ...
                        ; if BX != parity off, then BX = value to be
                        ; or-ed to AL to indicate parity setting
                        ; could be MOV BX, ENABLE_PARITY
                        ; OR BX, EVEN, PARITY 
                        ; to set up even parity
...
; line control register and BX as either a value to turn parity off,
; or a bit pattern to be or-ed with the LCR to enable a specific
; type of parity. If parity is off, nothing happens. If parity is on,
; a value is sent to the line control register to turn a specific type of 
; parity on/
;
