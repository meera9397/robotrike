...
;
; Revision History:
;    11/11/16  Meera Krishnamoorthy   wrote code, debugged
...
EXTRN   Sin_Table:WORD       ; table with sin values of angles 0 - 359
EXTRN   Cos_Table:WORD       ; table with cos values of angles 0 - 359

EXTRN   MotorForcesX_Table:WORD      ; table with components 
...
;              functions concerning the motors. It also writes to the control
; 			   port to ensure that Port B is an output port (this is the 
; 			   port we will be sending our motor signal to). 
;
; Operation: This is done by setting all shared variables created in the
; 			 data segment to zero, including each element in the pulseWidths
; 			 byte array. Then, this function writes a value to the control
; 			 port, setting the bits related to the function of port B to
; 		     0 to ensure that that port is an output port. 
;
; Arguments: None.
; Return Value: None.
;
...
;                    off
;                   4) pulseWidths[], W, type = bytes, length = (8 bits) *
...
;                   3) pulseWidths[], W, type = bytes, length = (8 bits) *
;                     NUM_MOTORS; pulseWidths is an array holding the power
;                     delivered to each motor. The index of pulseWidth
;                     corresponds to the motor, and pulseWidth[index]
;                     corresponds to the power delivered to that motor.
...
; Shared Variables: 1) pulseWidths[], W, type = bytes, length = (8 bits) *
;                     NUM_MOTORS; pulseWidths is an array holding the power
;                     delivered to each motor. The index of pulseWidth
;                     corresponds to the motor, and pulseWidth[index]
;                     corresponds to the power delivered to that motor.
