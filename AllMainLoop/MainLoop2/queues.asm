...
RemoveByte: ; dequeue byte from array
    ...
    MOV BX, [SI].front
    MOV CL, byte ptr [SI].elements[BX]
...
QueueNotFull:
    MOV BX, word ptr [SI].element_size ; checks the element size
    CMP BX, byte_len
...
AddWord:
    ...
    ;PUSHF 
    ;CLI
	MOV BX, word ptr [SI].rear ; get array index
	...
	MOV AX, BX
    MOV DX, 0
	DIV word ptr[SI].len ; accounts for wrapping (tail pointer = queue length + 2 = 0)
	MOV BX, DX
    
    MOV word ptr[SI].rear, BX
    ;POPF
    JMP End_Enqueue
